...
using Raspberry.IO.Interop.NetStandard.EnumTypes;
using Raspberry.System.NetStandard;
using Raspberry.System.NetStandard.Timers;

#endregion

namespace Raspberry.IO.GeneralPurpose.NetStandard
{
    /// <summary>
    ...
    public class GpioConnectionDriver : IGpioConnectionDriver
    {
        ...
        public void Allocate(ProcessorPin pin, PinDirection direction)
        {
            ...
            {
                if (!pinResistors.TryGetValue(pin, out var pinResistor) || pinResistor != PinResistor.None)
        ...
        {
            var offset = Math.DivRem((int)pin, 32, out var shift);
        ...
        private void InitializePoll(ProcessorPin pin)
        {
            lock (pinPolls)
            {
                if (pinPolls.TryGetValue(pin, out _))
                    return;
        ...
        {
            if (pinPolls.TryGetValue(pin, out var poll))
            {
                pinPolls.Remove(pin);
