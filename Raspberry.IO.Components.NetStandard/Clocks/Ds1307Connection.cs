...
namespace Raspberry.IO.Components.NetStandard.Clocks
{
    /// <summary>
    ...
    public class Ds1307Connection
    {
        ...
        private DateTime GetDate(byte[] input)
        {
            ...
            int seconds = input[0];
            if (!IsRtcEnabled(input[0])) seconds -= 128;  // Remove "CH"-bit from the seconds if present 

            seconds = NibbleToInt((byte)seconds);

            var minutes = NibbleToInt(input[1]);
            var hours = NibbleToInt(input[2]);
            ...
            var day = NibbleToInt(input[4]);
            var month = NibbleToInt(input[5]);
            var year = NibbleToInt(input[6]);
