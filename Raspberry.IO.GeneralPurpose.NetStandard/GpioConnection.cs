...
namespace Raspberry.IO.GeneralPurpose.NetStandard
{
    ...
    public class GpioConnection : IDisposable
    {
        ...
        /// </summary>
        /// <param name="e">The <see cref="PinStatusEventArgs"/> instance containing the event data.</param>
        ...
        private void Release(PinConfiguration configuration)
        {
            ...
            if (pinEvents.TryGetValue(configuration.Pin, out var handler))
            {
                PinStatusChanged -= handler;
        ...
        private void CheckInputPins()
        {
            ...
            foreach (var np in changes.Enumerate())
            {
                ...
                if (oldPinValue != newPinValue)
                {
                    ...
                    if (pin is SwitchInputPinConfiguration switchPin)
                    {
